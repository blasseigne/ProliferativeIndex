...
The ProliferativeIndex R package^[Ramaker and Lasseigne, et al. biorxiv citation] provides users with R functions for calculating and analyzing the proliferative index (PI) from an RNA-seq dataset.  
...
```
This dataset, `vstTCGA_ACCData` can be accessed from the package:
```{r, include=TRUE}
data(vstTCGA_ACCData)

#Examine only the first few columns and rows because the dataset is large (20501 genes x 79 samples):
dim(vstTCGA_ACCData)
#Note that sample IDs are column names and HGNC gene IDs (http://www.genenames.org) are rownames and that vst data is numeric.
str(vstTCGA_ACCData)
knitr::kable(vstTCGA_ACCData[1:5,1:5])
```
...
exampleTCGAData<-readDataForPI(vstTCGA_ACCData, c("AIFM3", "ATP9B", "CTRC", "MCL1", "MGAT4B", "ODF2L", "SNORA65", "TPPP2"))
